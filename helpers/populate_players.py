...
def get_stats_for_years(stats_groups, target_years):
    ...
    for year in target_years:
        results[str(year)] = {
            "hitting": {
                "HR": 0, "R": 0, "RBI": 0, "SB": 0,
                "BA": 0.0, "SLG": 0.0, "OBP": 0.0, "OPS": 0.0, "PA": 0,
            },
            ...
            if season in results:
                if group_name == "hitting":
                    results[season]["hitting"] = {
                        "HR": s.get("homeRuns", 0), "R": s.get("runs", 0),
                        "RBI": s.get("rbi", 0), "SB": s.get("stolenBases", 0),
                        "BA": to_f(s.get("avg", 0.0)), "SLG": to_f(s.get("slg", 0.0)),
                        "OBP": to_f(s.get("obp", 0.0)), "OPS": to_f(s.get("ops", 0.0)),
                        "PA": s.get("plateAppearances", 0),
                    }
                elif group_name == "pitching":
                    results[season]["pitching"] = {
                        "W": s.get("wins", 0), "K": s.get("strikeOuts", 0),
                        "SV": s.get("saves", 0), "ERA": to_f(s.get("era", 0.0)),
                        "WHIP": to_f(s.get("whip", 0.0)), "IP": to_f(s.get("inningsPitched", 0.0)),
                    }
...
def calculate_weighted_ranks(db):
    ...
    for p in players:
        ...
        is_hitter = any(pos in pos_list for pos in ["C", "1B", "2B", "3B", "SS", "OF", "UT"]) or not pos_list
            ...
            for metric in hitter_metrics:
                ...
                for year, weight in weights.items():
                    ...
                    if pa > 0:
                        ...
                        total_weight += weight
                processed["metrics"][metric] = total_val / total_weight if total_weight > 0 else -1
    ...
    for metric in all_metrics:
        valid_players = [p for p in player_stats_to_rank if metric in p["metrics"] and p["metrics"][metric] >= 0]
        ...
        for i, p_data in enumerate(valid_players):
            bulk_updates.append(UpdateOne({"_id": p_data["id"]}, {"$set": {f"statRanks.{metric}": i + 1}}))
...
def sync_mlb_players():
    try:
        ...
        for team in teams:
            ...
            try:
                d_res = requests.get(depth_url).json().get("roster", [])
                pos_counters = {}
                for entry in d_res:
                    pid = entry['person']['id']
                    ...
                    if abbr not in pos_counters: pos_counters[abbr] = 0
                    pos_counters[abbr] += 1
            ...
            # --- FETCH 40 MAN ---
            roster_url = f"https://statsapi.mlb.com/api/v1/teams/{t_id}/roster/40Man"
            try:
                roster_data = requests.get(roster_url).json().get("roster", [])
                for p in roster_data:
                    all_players_metadata[p["person"]["id"]] = {"fullName": p["person"]["fullName"], "teamId": t_id}
            except: continue
        ...
        for i in range(0, len(pids), batch_size):
            ...
            stats_url = f"https://statsapi.mlb.com/api/v1/people?personIds={ids_str}&hydrate=stats(group=[hitting,pitching,fielding],type=[yearByYear])"
            ...
            try:
                ...
                for person in response.get("people", []):
                    ...
                    live = depth_data.get(player_id, {"depthRanks": {}, "injuryStatus": "A"})

                    stats_groups = person.get("stats", [])
                    yearly_stats, pos_string = get_stats_for_years(stats_groups, ["2023", "2024", "2025"])

                    if not pos_string:
                        raw_primary = person.get("primaryPosition", {}).get("abbreviation")
                        pos_string = "OF" if raw_primary in ["LF", "CF", "RF", "OF"] else raw_primary
